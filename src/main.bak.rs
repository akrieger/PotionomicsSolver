...
#[derive(Debug, Parser)]
pub struct Args {
    #[arg(short, long, value_name="ingredients.rs")]
    ingredients: Option<String>,

    #[arg(short, long, value_enum, default_value_t=SolveAlgorithm::EXACT)]
    mode: SolveAlgorithm,
}
pub fn main() {
    let args = Args::parse();
    ...
    solve(
        ...
        &mut |result| {
            if print_mode == "all" || acc.len() == 0 {
                ...
                let candidate_total = result.1.magimins.total();
                let candidate_scaled_ratio = candidate_total / &target.magimins;
                ...
                let new_rms = rms(
                    scaled_expected_ratio.as_array(),
                    target.max,
                    candidate.magimins.as_array(),
                );
                if new_rms < current_rms {
                    println!(
                        "total: {}, a: {}, b: {}, c: {}, d: {}, e:{}",
                        candidate.magimins.total(),
                        candidate.magimins.a,
                        candidate.magimins.b,
                        candidate.magimins.c,
                        candidate.magimins.d,
                        candidate.magimins.e
                    );
                    acc.push(result);
                }
            }
